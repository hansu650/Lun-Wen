...
Adapted from VCIP-RGBD/DFormer:
...
class dformerv2(nn.Module):
    ...
    def init_weights(self, pretrained=None):
        if pretrained is None:
            self.apply(self._init_weights)
            return None
        return load_dformerv2_pretrained(self, pretrained)
...
def _normalize_dformerv2_key(key):
    prefixes = ("module.", "model.", "backbone.", "rgb_encoder.", "encoder.")
...
def load_dformerv2_pretrained(model, pretrained):
    ...
    for key, value in checkpoint.items():
        key = _normalize_dformerv2_key(key)
    ...
    unexpected_total = unexpected_count + shape_mismatch_count + len(result.unexpected_keys)
    print(f"[DFormerv2] loaded pretrained: {pretrained}")
    print(f"[DFormerv2] loaded_keys={len(state_dict)} missing_keys={missing_count} unexpected_keys={unexpected_total}")
    return {
        "loaded_keys": len(state_dict),
        "missing_keys": missing_count,
        "unexpected_keys": unexpected_total,
    }
