...
class CustomLayerDecayOptimizerConstructor(DefaultOptimizerConstructor):
    def add_params(self, params, module, prefix='', is_dcn_module=None):
        ...
        for name, param in module.named_parameters():
            ...
            group_name = "layer_%d_%s_%s" % (layer_id, group_name, group_name_custom)

            if group_name not in parameter_groups:
                scale = layer_decay_rate ** (num_layers - layer_id - 1)

                parameter_groups[group_name] = {
                    "weight_decay": this_weight_decay * decay_mult,
                    "params": [],
                    "param_names": [],
                    "lr_scale": scale * lr_mult,
                    "group_name": group_name,
                    "lr": scale * self.base_lr * lr_mult,
                }
